...
export class CreateComponent implements AfterViewInit {
	...
	replaceCommonTimeUnit(input: string): string {
		...
		if (["minutes", "minute", "min", "mins", "m"].includes(input))
			return "minute";
	...
	processMessage(input: string): Message {
		...
		let acceptableExpirationKeywords = [
			"expires",
			"expire",
			"deletes",
			"delete",
		...
		let acceptableExpirationViewUnits = [
			"views",
			"view",
			"v",
			"times",
		...
		let acceptableExpirationTimeUnits = [
			"minutes",
			"minute",
			"min",
			"mins",
		...
		configs.forEach((config) => {
			...
			config = config.replace("after", "").trim();
			...
			if (acceptableAliasKeywords.includes(command)) {
				...
				if (params.length < 3 || params.length > 20) {
					alert(
						"Alias can only have the length of 3 - 20 characters."
					);
					return;
				}
	...
	saveMessage() {
		...
		this._messageService.addMessage(data).subscribe({
			next: (doc: any) => {
				this.createdURLRef = doc.$id;
				this.isSubmitting = false;
				this.message = "";
				this.showComplete = true;
			},
			error: (error) => {
				if (error.error.type === "document_already_exists") {
					alert("Another message with the same ID already existed.");
					return;
				}

				console.log(error);
				alert(
					`Unable to create the secret message. Please try with another alias.\n\nPlease contact the developer if the problem persists.`
				);
				this.isSubmitting = false;
			},
		});
